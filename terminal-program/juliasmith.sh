#!/bin/sh
# juliasmith: Read the Word of God from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data juliasmith.tsv | awk -v cmd=list "$(get_data juliasmith.awk)"
		exit
	elif [ "$1" = "-W" ]; then
		export JULIASMITH_NOLINEWRAP=1
		shift
...
if [ $? -eq 0 ]; then
	export JULIASMITH_MAX_WIDTH="$cols"
fi
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data juliasmith.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data juliasmith.awk)" | ${PAGER}
	done
	exit 0
fi

get_data juliasmith.tsv | awk -v cmd=ref -v ref="$*" "$(get_data juliasmith.awk)" | ${PAGER}

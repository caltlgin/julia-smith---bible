...
URL='https://www.textusreceptusbibles.com/JuliaSmith/1/1'
NAME='julia-smith'
...
grep -Irl 'KJV' . | xargs sed -i 's/KJV/JULIASMITH/g'
grep -Irl 'kjv' . | xargs sed -i 's/kjv/juliasmith/g'
find . -name "kjv*" -exec rename -v 's/kjv/juliasmith/' {} ";"
...
mv "${NAME}.vpl" 'juliasmith.tsv'
make
./juliasmith 2 pet 3:9 | cat
popd
...
install -dv 'vpl'
pushd 'vpl'
cp "../terminal-program/juliasmith.tsv" "${NAME}.vpl"
